...
public class VentanaPrincipal extends Controlador {
	...
	void btBorrarAlquiler(ActionEvent event) {
		...
		ventanaBorrarAlquiler.getEscenario().setOnCloseRequest(ventanaBorrarAlquiler::ventanaCerrada);
	...
	void btBorrarCliente(ActionEvent event) {
		...
		ventanaBorrarCliente.getEscenario().setOnCloseRequest(ventanaBorrarCliente::ventanaCerrada);
	...
	void btBorrarVehiculo(ActionEvent event) {
		...
		ventanaBorrarVehiculo.getEscenario().setOnCloseRequest(ventanaBorrarVehiculo::ventanaCerrada);
	...
	void btBuscarAlquiler(ActionEvent event) {
		BuscarAlquiler ventanaBuscarAlquiler = (BuscarAlquiler) Controladores.get("vistas/buscarAlquiler.fxml",
				"Vista grafica de buscar alquiler", getEscenario());
		ventanaBuscarAlquiler.getEscenario().setOnCloseRequest(ventanaBuscarAlquiler::ventanaCerrada);
	...
	void btBuscarCliente(ActionEvent event) {
		BuscarCliente ventanaBuscarCliente = (BuscarCliente) Controladores.get("vistas/buscarCliente.fxml",
				"Vista grafica de buscar cliente", getEscenario());
		ventanaBuscarCliente.getEscenario().setOnCloseRequest(ventanaBuscarCliente::ventanaCerrada);
	...
	void btBuscarVehiculo(ActionEvent event) {
		BuscarVehiculo ventanaBuscarVehiculo = (BuscarVehiculo) Controladores.get("vistas/buscarVehiculo.fxml",
				"Vista grafica de buscar vehiculo", getEscenario());
		ventanaBuscarVehiculo.getEscenario().setOnCloseRequest(ventanaBuscarVehiculo::ventanaCerrada);
	...
	void btInsertarAlquiler(ActionEvent event) {
		...
		ventanaInsertarAlquiler.getEscenario().setOnCloseRequest(ventanaInsertarAlquiler::ventanaCerrada);
	...
	void btInsertarCliente(ActionEvent event) {
		...
		ventanaInsertarCliente.getEscenario().setOnCloseRequest(ventanaInsertarCliente::ventanaCerrada);
	...
	void btInsertarVehiculo(ActionEvent event) {
		...
		ventanaInsertarVehiculo.getEscenario().setOnCloseRequest(ventanaInsertarVehiculo::ventanaCerrada);
	...
	void btListarAlquiler(ActionEvent event) {
		...
		ventanaListarAlquileres.getEscenario().setOnCloseRequest(ventanaListarAlquileres::ventanaCerrada);
	...
	void btListarCliente(ActionEvent event) {
		...
		ventanaListarClientes.initialize(); // Es como tener un metodo especifico solo para actualizar e inicializar.
	...
	void btListarVehiculo(ActionEvent event) {
		...
		ventanaListarVehiculos.getEscenario().setOnCloseRequest(ventanaListarVehiculos::ventanaCerrada);
	...
	void desplegarMenu() {
		...
		TranslateTransition transicionDespliegue = new TranslateTransition(Duration.seconds(0.2),
				stackPaneMenuDesplegable);
	...
	void exportarPdf(ActionEvent event) {
		...
		try {
			...
			Dialogos.mostrarDialogoError("ERROR: Ocurrio un error al crear el archivo PDF", e.getMessage(),
					getEscenario());
		}
	}
	...
	void abrirPDF(String archivo) {
		try {
			...
			if (archivoPDF.exists()) {
				...
				Dialogos.mostrarDialogoError("ERROR: No se puede abrir el archivo PDF",
						"ERROR: El archivo PDF que desea abrir no existe", getEscenario());
			}

		} catch (IOException e) {
			Dialogos.mostrarDialogoError("ERROR: Ocurrio un error al abrir el archivo PDF", e.getMessage(),
					getEscenario());
		}
	}
	...
	PdfPTable crearTablaAlquileres() throws DocumentException {
		...
		PdfPCell celdaTituloAlquileres = new PdfPCell(
				new Paragraph("Alquileres", new Font(Font.FontFamily.HELVETICA, 12, Font.BOLD)));
		...
		for (Alquiler alquiler : listaAlquileres) {
			...
			tableAlquileres.addCell(new PdfPCell(new Phrase(
					(alquiler.getVehiculo() instanceof Turismo turismo) ? String.format("%s", turismo.getCilindrada())
							: "",
					fontTextoCelda)));
			tableAlquileres.addCell(new PdfPCell(new Phrase(
					(alquiler.getVehiculo() instanceof Furgoneta furgoneta) ? String.format("%s", furgoneta.getPma())
							: "",
					fontTextoCelda)));
			tableAlquileres.addCell(new PdfPCell(new Phrase(
					(alquiler.getVehiculo() instanceof Autobus autobus) ? String.format("%s", autobus.getPlazas())
							: (alquiler.getVehiculo() instanceof Furgoneta furgoneta)
									? String.format("%s", furgoneta.getPlazas())
									: "",
					fontTitulos)));

			tableAlquileres.addCell(
					new PdfPCell(new Phrase(String.format("%s", alquiler.getFechaAlquiler()), fontTextoCelda)));
			tableAlquileres.addCell(
					new PdfPCell(new Phrase(String.format("%s", alquiler.getFechaAlquiler()), fontTextoCelda)));
		}
	...
	PdfPTable crearTablaVehiculos() {
		...
		PdfPCell celdaTituloVehiculos = new PdfPCell(
				new Paragraph("Vehiculos", new Font(Font.FontFamily.HELVETICA, 12, Font.BOLD)));
		...
		for (Vehiculo vehiculo : listaVehiculos) {
			...
			tableVehiculos.addCell(new PdfPCell(new Phrase(
					(vehiculo instanceof Turismo turismo) ? String.format("%s", turismo.getCilindrada()) : "",
					fontTextoCelda)));
			tableVehiculos.addCell(new PdfPCell(
					new Phrase((vehiculo instanceof Furgoneta furgoneta) ? String.format("%s", furgoneta.getPma()) : "",
							fontTextoCelda)));
			tableVehiculos.addCell(new PdfPCell(new Phrase((vehiculo instanceof Autobus autobus)
					? String.format("%s", autobus.getPlazas())
					: (vehiculo instanceof Furgoneta furgoneta) ? String.format("%s", furgoneta.getPlazas()) : "",
					fontTitulos)));
		}
	...
	PdfPTable crearTablaClientes() {
		...
		PdfPCell celdaTituloClientes = new PdfPCell(
				new Paragraph("Clientes", new Font(Font.FontFamily.HELVETICA, 12, Font.BOLD)));
	...
	void ratonPresionaBotonVista(MouseEvent event) {
		...
		if (!stackPaneMenuDesplegable.getChildren().isEmpty()) {
			...
			Button botonMinimizar = null;
			Button botonMaximizar = null;
			...
			if (os.contains("nix") || os.contains("nux") || os.contains("aix")) {
				botonMinimizar = crearBotonesMenu();
				botonMaximizar = crearBotonesMenu();
	...
	void ratonPulsaMaximizar() {
		...
		Dialogos.mostrarDialogoInformacion("Boton Maximizar",
				"AVISO: Este boton no sirve para nada, ya que su funcionalidad se implementará en el siguiente spring",
				getEscenario());
	}
	...
	void ratonPulsaCambiarColor() {
		...
		getEscenario().getScene().getRoot().getStylesheets()
				.add(LocalizadorRecursos.class.getResource("css/VentanaPrincipal.css").toExternalForm());
	...
	void ratonPulsaCambiarColorNegro() {
		if (!getEscenario().getScene().getRoot().getStylesheets().contains("css/VentanaPrincipal-Negro.css")) {
			...
			getEscenario().getScene().getRoot().getStylesheets()
					.add(LocalizadorRecursos.class.getResource("css/VentanaPrincipal-Negro.css").toExternalForm());
			getEscenario().getScene().getRoot().requestLayout();
		}
	}
	...
	void ratonPulsaCambiarColorBlanco() {
		if (!getEscenario().getScene().getRoot().getStylesheets().contains("css/VentanaPrincipal-Blanco.css")) {
			...
			getEscenario().getScene().getRoot().getStylesheets()
					.add(LocalizadorRecursos.class.getResource("css/VentanaPrincipal-Blanco.css").toExternalForm());
			getEscenario().getScene().getRoot().requestLayout();
		}
	}
	...
	void ratonPulsaMostrarEstadisticas() {
		MostrarEstadisticasMensuales ventanaMostrarEstadisticasMensuales = (MostrarEstadisticasMensuales) Controladores
				.get("vistas/mostrarEstadisticasMensuales.fxml", "Vista grafica de las estadisticas mensuales",
						getEscenario());
		stackPaneMenuDesplegable.getChildren().clear();
	...
	@FXML
	void ratonPresionaHyperlinkGithubV0(ActionEvent event) {
		abrirNavegadorLink("https://github.com/Serferman/AlquilerVehiculos-v0");
	}

	@FXML
	void ratonPresionaHyperlinkGithubV1(ActionEvent event) {
		abrirNavegadorLink("https://github.com/Serferman/AlquilerVehiculos-v1");
	}

	@FXML
	void ratonPresionaHyperlinkGithubV2(ActionEvent event) {
		abrirNavegadorLink("https://github.com/Serferman/AlquilerVehiculos-v2");
	}

	@FXML
	void ratonPresionaHyperlinkGithubV3(ActionEvent event) {
		abrirNavegadorLink("https://github.com/Serferman/AlquilerVehiculos-v3");
	}

	@FXML
	void ratonPresionaHyperlinkGithubV4(ActionEvent event) {
		abrirNavegadorLink("https://github.com/Serferman/AlquilerVehiculos-v4");
	}
